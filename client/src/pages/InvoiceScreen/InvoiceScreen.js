...
const InvoiceScreen = () => {
  ...
  const cgst = invoice?.pricing?.cgstPrice || invoice?.pricing?.taxPrice / 2 || 0;
  const sgst = invoice?.pricing?.sgstPrice || invoice?.pricing?.taxPrice / 2 || 0;
  ...
  const summaryRows = invoice
    ? [
        { label: "Taxable Amount",   value: formatINR(taxableAmount), bold: true,  dark: false, green: false },
        { label: "CGST @2.5%",       value: formatINR(cgst),          bold: false, dark: false, green: false },
        { label: "SGST @2.5%",       value: formatINR(sgst),          bold: false, dark: false, green: false },
        { label: "Shipping Charges", value: formatINR(shipping),       bold: false, dark: false, green: false },
        ...(discountAmount > 0
          ? [{
          ...
          : []),
        { label: "Total Amount",     value: formatINR(total),          bold: true,  dark: true,  green: false },
      ]
    : [];
  ...
  const checkPageBreak = (currentY, neededHeight = 10) => {
    if (currentY + neededHeight > pageHeight - marginBottom) {
      ...
      doc.setTextColor(80);
      doc.text(FROM_NAME + "  |  Invoice #" + (invoice.invoiceNumber || "N/A"), 14, 9);
      doc.setTextColor(0);
  ...
  let logoBase64 = null;
  try {
    logoBase64 = await getImageBase64(stampLogo);
  } catch (err) {
    console.log(err);
  }
  ...
  const pdfTaxable =
    ...
    (invoice.pricing?.taxPrice || 0) -
    (invoice.pricing?.shippingPrice || 0);
  const pdfCgst = invoice.pricing?.cgstPrice || invoice.pricing?.taxPrice / 2 || 0;
  const pdfSgst = invoice.pricing?.sgstPrice || invoice.pricing?.taxPrice / 2 || 0;
  ...
  const pdfSummaryRows = [
    { label: "Taxable Amount",   value: formatPDF(pdfTaxable),  dark: false, bold: true,  green: false },
    { label: "CGST @2.5%",       value: formatPDF(pdfCgst),     dark: false, bold: false, green: false },
    { label: "SGST @2.5%",       value: formatPDF(pdfSgst),     dark: false, bold: false, green: false },
    { label: "Shipping Charges", value: formatPDF(pdfShipping), dark: false, bold: false, green: false },
    ...(pdfDiscountAmount > 0
      ? [{
      ...
      : []),
    { label: "Total Amount",     value: formatPDF(pdfTotal),    dark: true,  bold: true,  green: false },
  ];
  ...
  doc.setFont("helvetica", "normal");
  const splitWords = doc.splitTextToSize(numberToWords(Math.floor(pdfTotal)), 170);
  ...
  for (let p = 1; p <= totalPages; p++) {
    ...
    doc.text(`Page ${p} of ${totalPages}`, pageWidth - 14, 291, { align: "right" });
  }
  ...
  return (
    <Box p={6} maxW="860px" mx="auto">
      ...
      <Flex justify="space-between" align="center" mb={6}>
        <Text fontSize="xl" fontWeight="semibold">Invoice Preview</Text>
      ...
      {loading ? (
        ...
        <Box border="1px solid" borderColor="gray.200" borderRadius="xl" overflow="hidden" boxShadow="sm">

          {/* Header */}
          <Box bg="gray.50" px={6} py={5} borderBottom="1px solid" borderColor="gray.200">
            <Flex align="center" gap={4}>
              <Box w="56px" h="56px" borderRadius="full" overflow="hidden" flexShrink={0} border="1px solid" borderColor="gray.200">
                <img src={stampLogo} alt="Viyavar Fashions" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
              </Box>
              <Box flex={1}>
                <Text fontWeight="semibold" fontSize="lg">{FROM_NAME}</Text>
                <Text fontSize="xs" color="gray.500" lineHeight={1.6}>
                  {FROM_ADDRESS.join(", ")}<br />GSTIN: {GST_NUMBER}
                </Text>
              </Box>
              <Box textAlign="right">
                <Text fontSize="sm" fontWeight="medium">INV #{invoice.invoiceNumber || "N/A"}</Text>
                <Text fontSize="xs" color="gray.500" mt={1}>
                  {invoice.createdAt
                    ? new Date(invoice.createdAt).toLocaleDateString("en-IN")
                    : ""}
                </Text>
                <Box display="inline-block" mt={2} px={3} py={1} bg="green.50" color="green.600" borderRadius="full" fontSize="11px" fontWeight="medium">
                  ✓ Invoice
                </Box>
              </Box>
            </Flex>
          ...
          <Box px={6} py={5}>
            ...
            <Grid templateColumns="1fr 1fr" gap={4} mb={6}>
              <Box bg="gray.50" borderRadius="lg" p={4}>
                <Text fontSize="11px" fontWeight="bold" color="gray.700" letterSpacing="wide" mb={2} textTransform="uppercase">From</Text>
                <Text fontSize="sm" fontWeight="bold" color="gray.800">{FROM_NAME}</Text>
                <Text fontSize="xs" color="gray.500" lineHeight={1.7}>
                  {FROM_ADDRESS.map((l, i) => <span key={i}>{l}<br /></span>)}
                  GSTIN: {GST_NUMBER}
                </Text>
              </Box>
              <Box bg="gray.50" borderRadius="lg" p={4}>
                <Text fontSize="11px" fontWeight="bold" color="gray.700" letterSpacing="wide" mb={2} textTransform="uppercase">To</Text>
                <Text fontSize="sm" fontWeight="bold" color="gray.800">{invoice.user?.name || "N/A"}</Text>
                <Text fontSize="xs" color="gray.500" lineHeight={1.7}>
                  {invoice.shippingAddress?.doorNo} {invoice.shippingAddress?.street}<br />
                  {invoice.shippingAddress?.city}, {invoice.shippingAddress?.state} – {invoice.shippingAddress?.pin}<br />
                  {invoice.shippingAddress?.phoneNumber}
                </Text>
              </Box>
            </Grid>
            ...
            <Box overflowX="auto" mb={6}>
              <Table size="sm" variant="simple">
                <Thead>
                  <Tr>
                    {["Item", "HSN", "Qty", "Size", "Price", "Total"].map((h) => (
                      <Th key={h} fontSize="10px" color="white" py={3} bg="gray.800">{h}</Th>
                    ))}
                  </Tr>
                </Thead>
                <Tbody>
                  {invoice.orderItems.map((item, i) => (
                    <Tr key={i} bg={i % 2 === 0 ? "white" : "gray.50"}>
                      <Td>
                        ...
                        <Text fontSize="xs" color="gray.400">HSN: {item.hsnCode || "6109"}</Text>
                      </Td>
                      <Td fontSize="sm">{item.hsnCode || "6109"}</Td>
                      <Td fontSize="sm" textAlign="center">{item.qty}</Td>
                      <Td fontSize="sm" textAlign="center">{item.size || "-"}</Td>
                      <Td fontSize="sm" isNumeric>₹ {formatINR(item.price)}</Td>
                      <Td fontSize="sm" fontWeight="medium" isNumeric>₹ {formatINR(item.qty * item.price)}</Td>
                    </Tr>
                  ))}
                </Tbody>
              </Table>
            ...
            <Flex justify="flex-end" mb={6}>
              <Box w="320px" border="1px solid" borderColor="gray.200" borderRadius="lg" overflow="hidden">
                {summaryRows.map((row, i) => (
                  <Flex
                    ...
                    borderBottom={i < summaryRows.length - 1 ? "1px solid" : "none"}
                    borderColor="gray.100"
                  >
                    <Text
                      fontSize="sm"
                      fontWeight={row.bold ? "bold" : "normal"}
                      color={row.dark ? "white" : row.green ? "green.700" : "gray.600"}
                    >
                      {row.label}
                    </Text>
                    <Text
                      ...
                      color={row.dark ? "white" : row.green ? "green.700" : "gray.800"}
                    >
                      {row.green ? "- " : ""}₹ {row.value}
                    </Text>
                  </Flex>
          ...
          {/* Footer */}
          <Box bg="gray.50" borderTop="1px solid" borderColor="gray.200" py={3} textAlign="center">
